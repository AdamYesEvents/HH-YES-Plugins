...
(function () {
  ...
  // After a batch save HireHop rebuilds the jstree (30-40s on large jobs); during
  // that rebuild select_node silently no-ops and get_selected returns []. Every
  // subsequent parent-dependent write (save_items_list, insertCustoms) then
  // targets the wrong node. Poll BEFORE those writes, don't wait for the widget
  // to figure it out on its own. Timeout is generous - we'd rather add 30s of
  // waiting than send items to the wrong folder.
  var NODE_READY_TIMEOUT_MS = 45000;
  function waitForNodeSelectable(inst, subHeadingId, cb) {
    var start = Date.now(), nodeId = "a" + subHeadingId;
    (function poll() {
      var t = inst.items_to_supply_tree.jstree(true);
      try {
        t.deselect_all();
        t.select_node(nodeId);
        var sel = t.get_selected();
        if (sel && sel.length && String(sel[0]) === nodeId) return cb(true);
      } catch (e) { }
      if (Date.now() - start > NODE_READY_TIMEOUT_MS) return cb(false);
      setTimeout(poll, 500);
    })();
  }
